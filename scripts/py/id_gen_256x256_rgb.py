...
def readDeepNet(trainNet_path, caffemodel_path, proj_root):
    ...
    #transformer.set_input_scale('data', 0.0034062)
...
def extract_feature(net, transformer, filelist, dimension, imageSize, data_folder):
    ...
    while 1:
        ...
        out = net.forward()
        feat = net.blobs['fc5'].data[0] # 'fc256' is the layer name in caffemodel, edit this arguments base on your own model
...
if __name__ == "__main__":
    ...
    data_folder = "/home/zeng/data4my_dirtywork/id_rgb_256/"
    ...
    (feature, label) = extract_feature(net, transformer, filelist, 512, 50, data_folder)
    outputFileName =  testname + "_intra.npy"
    print(feature.shape)
    ...
    (feature, label) = extract_feature(net, transformer, filelist, 512, 50, data_folder)
    outputFileName =  testname + "_extra.npy"
    print(feature.shape)
    np.save(outputFileName, feature)

    '''

    filelist = data_folder + "_test/cacd_withNoLFWebface.txt"
    (feature, label) = extract_feature(net, transformer, filelist, 256, 50)
    outputFileName = data_folder + "_test/cacd112x96_feature.npy"
    print(feature.shape)
    np.save(outputFileName, feature)

    np.save(data_folder + "_test/cacd112x96_label.npy", label)
    '''
