...
def get_dataset(split_name='train', dataset_dir='/data/zeng/tf_webface_256x256', file_pattern=None, reader=None):
  ...
  items_to_handlers = {
      'img_raw': slim.tfexample_decoder.Image(shape=[256,256,3]),
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      [image, label] = provider.get(['img_raw', 'label'])
      label -= FLAGS.labels_offset

      train_image_size = FLAGS.train_image_size or network_fn.default_image_size

      image = image_preprocessing_fn(image, 256, 256)
